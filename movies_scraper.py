...
def search_movies(query):
    ...
    try:
        ...
        movies = website.find_all("a", {'class': 'film-poster-ahref'})  # Adjust class name accordingly
        print(f"[DEBUG] Found Movies: {len(movies)}")
        
        for index, movie in enumerate(movies):
            movie_details = {}
            title = movie['title']
            poster_img = movie.find("i", {'class': 'fa fa-play'}).parent.get('href', '')  # Adjust based on actual HTML structure
            
            movie_details["id"] = f"link{index}"
            movie_details["title"] = title
            movie_details["image"] = poster_img  # Image URL
